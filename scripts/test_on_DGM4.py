...
def evaluate_model(test_loader, model, processor,device,option_vectors,vectorizer,options,option_labels,tokenizer):
    ...
    with torch.inference_mode():
        for inputs, batch_answers,fake_text_pos_list,captions,fake_image_box,image_sizes in tqdm(test_loader, desc="Evaluating"):
            ...
            _, _, best_multi_labels, pred_label = get_best_option(
                task_answers, option_vectors, vectorizer, options, option_labels, device
            )
        
            ##--reeal/fake---##
            cls_nums_all += len(batch_answers)
            ...
            ## token-acc ##
            token_acc_sum += compute_token_acc(captions, pred_words_list, fake_text_pos_list, tokenizer)
            token_acc_count += 1
        

    IOU_score = iou_sum / max(iou_count, 1)
    Token_ACC = token_acc_sum / max(token_acc_count, 1)
    ACC_cls = cls_acc_all / max(cls_nums_all, 1)
    
    MAP = multi_label_meter.value()[:3].mean()
    
    return ACC_cls, cls_acc_all, cls_nums_all, MAP, IOU_score, Token_ACC
