...
class Florence2LanguageModel(Florence2LanguagePreTrainedModel):
    ...
    def get_encoder_output_newEmbedding(
        ...
        if encoder_outputs is None: ## 源码正确
            ...
            if first_forward:
                ...
                depart_len = 577 + self.learnable_tokens_len
            ...
            output_learnable_token = first_encoder_outputs[0][:,577:577+self.learnable_tokens_len,:] ##[bs,32,768]
    ...
    def orthogonal_loss(self, X):
        """
        约束输入张量 X 的行向量尽可能正交，以增加矩阵的秩，使其接近 len。
        (经过实验，发现，如果只加入这一个约束，可能会导致AT分布棋盘化，丢失信息)
        (或许应该配合模约束来制止这种行为)
        参数：
            X (torch.Tensor): 形状为 [bs, len, 768] 的输入张量。
        
        返回：
            loss (torch.Tensor): 约束损失。
        """
        
        # 1. 计算行向量的 Gram 矩阵 G = X X^T，形状 [bs, len, len]
        #    G[i, j] 代表 X 的第 i 行和第 j 行的点积
        G = torch.matmul(X, X.transpose(1, 2))  # [bs, len, len]
        
        # 2. 提取 G 的对角线，即每个行向量自身的范数平方
        G_diag = torch.diagonal(G, dim1=-2, dim2=-1)  # [bs, len]
        
        # 3. 构造对角矩阵
        G_diag_matrix = torch.diag_embed(G_diag)  # [bs, len, len]
        
        # 4. 计算 Frobenius 范数损失
        #    目标：让非对角元素尽可能接近 0，使行向量正交
        loss = torch.norm(G - G_diag_matrix, p='fro', dim=[-2, -1]) ** 2
    ...
    def magnitude_loss(self, X, eps=1e-6):
        """
        通过KL散度实现模约束, 当Token内特征均匀分布时
        KL散度接近0, 这时loss也会降低到接近0的值。
        
        约束每个 token 内部特征的“能量”分布更加均匀，
        使得各个维度的贡献更为平衡，避免出现“棋盘化”分布而丢失信息。
        
        
        参数：
            X (torch.Tensor): 形状为 [bs, len, 768] 的输入张量。
            eps (float): 防止除 0 的小常数。
            
        返回：
            loss (torch.Tensor): 模损失。
        """
        # 1. 计算每个 token 的每个维度能量（平方）， 逐元素平方
        X_squared = X ** 2  # shape: [bs, len, 768]
        
        # 2. 对每个 token 内部归一化，使得能量和为 1
        token_energy = X_squared.sum(dim=-1, keepdim=True) + eps  # 计算token内部的总能量 token_energy = [bs, len, 1]
        p = X_squared / token_energy  # 每个特征维度归一化， p=[bs, len, 768] 
        
        # 3. 构造均匀分布：所有维度目标概率均为 1/768
        d = X.shape[-1]
        uniform = 1.0 / d  # 标量，因为每个维度目标一致
        
        # 4. 计算每个 token 内部的 KL 散度：p * (log(p) - log(1/d))
        # # p * (torch.log(p + eps) - math.log(uniform))得到的形状依然是[bs,len,768]
        # # 经过.sun(dim=-1)得到的loss是[bs,len]，即每个token的KL散度损失
        loss = (p * (torch.log(p + eps) - math.log(uniform))).sum(dim=-1)
        
        #调用mean()返回，如果不指定维度，mean()会对所有维度求平均，并返回一个标量
        return loss.mean()
    ...
    def forward(
        ...
        # import traceback
        # print(f'在Florence2LanguageModel的调用盏是')
        ...
        if encoder_outputs is None: ## 源码正确
            ...
            ### 正则化放在第一次Forward之后
            # # loss_regular = loss_正交 + 0.2*loss_模
            ## 并且，我打算让loss_regular占的比例小一点，在总损失中占0.05
            # loss_regular = self.orthogonal_loss(output_learnable_token)+ 0.2*(self.magnitude_loss(output_learnable_token))
            
            
            # #<<----------不同的learnable Token处理方法---------->>
            ## <-----1. 直接平均法------> 在NYT上效果不好
            ##使用learnable_token做平均得到[]，做一个二分类，
            # mean_learnable_token = output_learnable_token.mean(dim=1)  # [bs, 768]
            
            ## <-----2. 加权注意力聚合法------>
            # [batch_size, num_tokens, input_dim] -> [batch_size, num_tokens, hidden_dim]
            attn_scores = torch.relu(self.attn_linear(output_learnable_token))  # 对每个token做映射
            ...
            mean_learnable_token = torch.sum(attn_weights * output_learnable_token, dim=1) 
        ...
        Image_LearnableToken_emb = last_hidden_state[:,:577 + self.learnable_tokens_len,:] # # image(577)+learnableToken(32)组合
        Text_emb = last_hidden_state[:,577 + self.learnable_tokens_len:,:] # # text做一个组合
        # 将图像编码作为 query，文本编码作为 key 和 value，进行自注意力机制计算
        image_attention_output = self.attention(Image_LearnableToken_emb, Text_emb, Text_emb)
...
class Florence2LanguageForConditionalGeneration(Florence2LanguagePreTrainedModel):
    ...
    def forward(
        self,
        input_ids: torch.LongTensor = None,
        learnable_forward: Optional[bool] = True, ##是否是第一次prompt
        attention_mask: Optional[torch.Tensor] = None,
        decoder_input_ids: Optional[torch.LongTensor] = None,
        decoder_attention_mask: Optional[torch.LongTensor] = None,
        head_mask: Optional[torch.Tensor] = None,
...
class Florence2ForConditionalGeneration(Florence2PreTrainedModel):
    ...
    def generate(
        ...
        **kwargs
        ...
        #     input_ids=None,
        #     inputs_embeds=inputs_embeds,
        #     learnable_forward = learnable_forward,
        #     **kwargs
        # )
